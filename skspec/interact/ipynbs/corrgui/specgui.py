...
class SpectraGui(Box):
	...
	# RUN HTML
        ...
        kwargs["children"] = [HBox([
	                        VBox([self.INOUT_panel(),
	                        self.model,
	                                  alert]),
	                        VBox([self.plot_panel(),
	                                  self.slicing_panel(),
	                                  self.unit_panel(),
	                                                   ]),
	                        ])]
	...
	takeout = Checkbox(description = 'takeit')
	link((self.model, "take"), (takeout,"value"))	
...
#        link((self.model, "load_file"), (loadfile, "value"))
	...
	filename = Text(description = "")
        link((self.model, "file_name"), (filename, "value"))		
	savetake = Button(color='black',background_color='AliceBlue',description='')
	saveplot.on_click(lambda x: self.model.take_out())	
	
        takename = Text(description = "")
        takebutton = Button(color='black',
	                    background_color='AliceBlue',
	                    description="Take SON"
	                    )
        #takebutton.on_click(lambda x: self.model.load_from_ns())	
	
	boxl = HBox([takename,takebutton])
	link((self.model, "take"), (boxl,"visible"))
	...
	link((self.model, "Standardplot"),(Splot,"value"))
