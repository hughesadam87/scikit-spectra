...
from pyuvvis.pandas_utils.dataframeserial import df_loads, df_dump
...
if __name__=='__main__':
 ...
 #   df=from_gwu_chem_IR(filelist, sortnames=True)
    #data=pkgutil.get_data('pyuvvis', 'data/gwuspecdata/11_data.txt')
    #time=pkgutil.get_data('pyuvvis','data/gwuspecdata/11_hours_timefile.txt')
    ...
    #df=from_timefile_datafile('data', 'time')
    
    #df_dump(df, 'spectra.pickle')
    
    
    df_stream=pkgutil.get_data('pyuvvis', 'data/example_data/specdata.pickle') 
