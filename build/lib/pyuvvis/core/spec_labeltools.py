...
from_T={'t':lambda x: x,  '%t':lambda x: 100.0 * x, 'r':lambda x:1.0/x, 'a':lambda x: -np.log10(x), 'a(ln)':lambda x:-np.log(x)}
to_T={'t':lambda x: x,  '%t':lambda x: x/100.0, 'r':lambda x:x*1.0, 'a':lambda x: np.power(10, -x), 'a(ln)':lambda x: np.exp(-x)}
